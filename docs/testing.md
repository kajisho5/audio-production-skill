...
| `tests/test_contract.py` | contract ⇔ implementation, `skill` = `contract`, doctor statuses (`supported` / `unsupported` / `unknown`), doctor without ffmpeg-skill, capability table, `contract --check` (pinned `tests/contract/contract.json`, breaking / additive classification, CLI exit codes) |
| `tests/test_integration.py` | per operation positive + negative (GAIN, TRIM incl. compressed and video sources, CUT, SILENCE_REMOVE, FADE_IN/OUT, NORMALIZE, MONO, STEREO, DOWNMIX, NOISE_REDUCTION, DYNAMICS, CONCAT with and without crossfade, every output format incl. the missing-encoder path), the real-audio pipeline `TRIM → GAIN → FADE_OUT → NORMALIZE`, two-source `MIX → NORMALIZE` (with implicit track range / gain and a muted third input), dry run writes nothing, re-run reuse + `--no-reuse` + tampered intermediate, invalid inputs, expectation failures remove the output, loudness verification failure, timeout, SIGINT cancellation, CLI validate / exit codes / human output |
