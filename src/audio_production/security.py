...
- Inputs must be existing regular files; symlinks are resolved before every check so a link cannot escape a root.
...
- Nothing in a request ever becomes an executable, a command, an argv fragment or a filter string; executables are
  found by the ffmpeg-skill adapter only (see adapter.py)."""
...
def check_filename(name: str) -> None:
    ...
    if not name or name in (".", ".."):
        raise AudioError("PATH_NOT_ALLOWED", f"unsafe file name: {name!r}", {"reason": "empty_or_dot"})
    if len(name.encode("utf-8", "replace")) > MAX_NAME_LENGTH:
        raise AudioError("PATH_NOT_ALLOWED", "file name is too long", {"reason": "name_too_long"})
    if INVALID_NAME_CHARS.search(name):
        raise AudioError("PATH_NOT_ALLOWED", f"file name contains an invalid character: {name!r}", {"reason": "invalid_character"})
    if name.endswith(" ") or name.endswith("."):
        raise AudioError("PATH_NOT_ALLOWED", f"file name may not end with a space or a dot: {name!r}", {"reason": "trailing_space_or_dot"})
    stem = name.split(".", 1)[0].upper()
    if stem in WINDOWS_RESERVED:
        raise AudioError("PATH_NOT_ALLOWED", f"file name is a reserved device name: {name!r}", {"reason": "reserved_name"})
    if name.startswith("-"):
        raise AudioError("PATH_NOT_ALLOWED", f"file name may not start with '-': {name!r}", {"reason": "option_like_name"})


def _check_path_string(path: object, what: str) -> str:
    if not isinstance(path, str) or not path:
        raise AudioError("INVALID_REQUEST", f"{what} must be a non-empty path string", {"field": what})
    if "\x00" in path:
        raise AudioError("PATH_NOT_ALLOWED", f"{what} contains a NUL byte", {"reason": "nul_byte"})
    if len(path) > MAX_PATH_LENGTH:
        raise AudioError("PATH_NOT_ALLOWED", f"{what} is too long", {"reason": "path_too_long"})
    return path
...
class PathPolicy:
    def __init__(self, workspace: Optional[str] = None, allowed_input_roots: Optional[List[str]] = None):
        ...
        if not self.workspace.is_dir():
            raise AudioError("PATH_NOT_ALLOWED", f"workspace is not a directory: {self.workspace}", {"reason": "workspace_missing"})
        ...
        for r in self.allowed_input_roots or []:
            if not r.is_dir():
                raise AudioError("PATH_NOT_ALLOWED", f"allowed input root is not a directory: {r}", {"reason": "root_not_directory"})
    ...
    def resolve_input(self, path: object, what: str = "input") -> Path:
        ...
        try:
            ...
            raise AudioError("INVALID_INPUT", f"{what} not found: {text}", {"reason": "not_found", "path": text})
        except (OSError, RuntimeError) as e:
            raise AudioError("INVALID_INPUT", f"cannot resolve {what} path: {e}", {"reason": "unresolvable", "path": text})
        if not resolved.is_file():
            raise AudioError("INVALID_INPUT", f"{what} is not a regular file: {text}", {"reason": "not_regular_file", "path": text})
        if self.allowed_input_roots is not None and not any(_under(resolved, r) for r in self.allowed_input_roots):
            raise AudioError("PATH_NOT_ALLOWED", f"{what} is outside the allowed input roots: {text}",
        ...
        if not os.access(str(resolved), os.R_OK):
            raise AudioError("INVALID_INPUT", f"{what} is not readable: {text}", {"reason": "not_readable", "path": text})
        return resolved
    ...
    def resolve_write_path(self, path: object, what: str = "output", allow_dir: bool = False) -> Path:
        ...
        if any(part == ".." for part in Path(text).parts):
            raise AudioError("PATH_NOT_ALLOWED", f"{what} path may not contain '..': {text}", {"reason": "traversal"})
        probe = target
        ...
        try:
            ...
            raise AudioError("PATH_NOT_ALLOWED", f"cannot resolve {what} path: {e}", {"reason": "unresolvable"})
        resolved = base / target.relative_to(probe) if probe != target else base
        if not _under(resolved, self.workspace):
            raise AudioError("PATH_NOT_ALLOWED", f"{what} is outside the workspace: {text}",
        ...
        if resolved.exists() and not (resolved.is_dir() if allow_dir else resolved.is_file()):
            raise AudioError("OUTPUT_ERROR", f"{what} exists and is not a regular {'directory' if allow_dir else 'file'}: {text}", {"reason": "wrong_kind"})
        return resolved
